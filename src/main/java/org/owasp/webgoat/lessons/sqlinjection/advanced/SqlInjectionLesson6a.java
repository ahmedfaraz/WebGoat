...
public class SqlInjectionLesson6a implements AssignmentEndpoint {
    ...
    public AttackResult injectableQuery(String accountName) {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            try (ResultSet results = ps.executeQuery()) {
                return executeSqlInjection(results, usedUnion, query);
            }
        } catch (Exception e) {
            return failed(this)
    ...
    private boolean unionQueryChecker(String accountName) {
        // Note the double backslash for \s inside a Java string literal
        return accountName.matches("(?i)(^[^-/*;)]*)(\\s*)UNION(.*$)");
    }

    /**
     * Replacement for the original executeSqlInjection: it now works on a ResultSet
     * produced by a PreparedStatement instead of executing a raw SQL string.
     */
    private AttackResult executeSqlInjection(ResultSet results, boolean usedUnion, String query)
        throws SQLException {
    ...
    private AttackResult verifySqlInjection(
        ...
        if (!(output.toString().contains("dave") && output.toString().contains("passW0rD"))) {
            return failed(this)
                .output(output.toString() + YOUR_QUERY_WAS + query)
                .build();
        }
